    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 backface-hidden bg-white p-3 shadow-inner drop-shadow-xl flex flex-col items-center">
          <div
            className={`w-full h-32 ${colorClass} bg-opacity-20 flex items-center justify-center mb-4 overflow-hidden relative`}
          >
          ...
          <div className="px-2 py-2 bg-white">
            <div className="flex justify-center space-x-1">
              ...
              <div className="w-6 h-0.5 bg-rose-200 rounded-full"></div>
              <div className="w-10 h-0.5 bg-rose-200 rounded-full"></div>
            </div>
            <div className="flex justify-center space-x-1 mt-1.5">
              <div className="w-12 h-0.5 bg-rose-200 rounded-full"></div>
        ...
        {/* Back (Message Side) */}
        <div
          className="absolute inset-0 backface-hidden bg-white p-4 shadow-xl flex flex-col justify-center text-center border-2 border-rose-200/70 overflow-hiden"
          style={{ transform: "rotateY(180deg)" }}
        >
          <div className="h-36 overflow-auto flex items-start">
            <p className="text-sm text-gray-800 font-medium leading-relaxed my-auto">
              {text}
            </p>
          </div>
        </div>
...
export default function ComplimentsScreen({ onNext }) {
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
    >
      <h2 className="text-4xl md:text-5xl text-balance font-dancing font-semibold text-rose-600 mb-8 z-10 text-center">
        {complimentsHeading}
      </h2>

      <div className="relative w-full max-w-md h-96 flex items-center justify-center">
        <div className="absolute top-0 max-[366px]:left-0 left-3 md:left-14 z-10 -rotate-6">
          <Polaroid text={compliments[0]} colorClass="bg-rose-400" />
        </div>
        <div className="absolute top-3 md:top-4 max-[366px]:right-0 right-4 md:right-14 z-20 rotate-12">
          <Polaroid text={compliments[1]} colorClass="bg-pink-400" />
        </div>
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 -rotate-2">
          <Polaroid text={compliments[2]} colorClass="bg-rose-300" />
        </div>
      </div>

      <motion.p
        className="text-rose-500/55 text-sm mt-4 animate-pulse"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 1 }}
      >
